...
update.CorrectCoef <- function(X0.glist, X1.glist, Theta.list, 
  ...
  for (j in c(1: p)) {
    ...
    for (g in c(1: G)) {
      ...
      tmp1.gvec[g] <- tmp1_g
      tmp2_g <- 2 * Theta.list[[g]][j, j] / (N_gvec[g] * Sigma_g[j]^2) * sum(X1.glist[[g]]^2)
      tmp2.gvec[g] <- tmp2_g
    }
    ...
    for (g in c(1: G)) {
      ...
      tmp3.gvec[g] <- tmp3_g
      tmp4_g <- 2 * N1_gvec[g] * Theta.list[[g]][j, j] / (N_gvec * Sigma_g[j]^2)
      tmp4.gvec[g] <- tmp4_g
    }
...
getRefbat <- function(Data, batch) {
  ...
  ggplot(data = rsd.cumfreq, aes(x = rsd, y = cumfreq)) +
    geom_line(aes(color = batch), size = linesize) +
...
# ----------------------------------------
CordBat <- function(X, batch, group, ref.batch, eps = 1e-5) {
  ...
  for (i in c(1: length(COM))) {
    ...
    for (g in c(1: group.num)) {
      Xb0.gi.COMi <- X.delout[batch.new == ref.batch & group.new == grp.level[i], metID]
    ...
    if (length(metID) > 5) {
      ...
      rhos <- rep(0, group.num)
      for (g in c(1: group.num)) {
        rho_g <- selrho.useCVBIC(Xb0.COMi.glist[[g]])
        rho_g <- rho_g[1]
        rhos[g] <- rho_g
      }
